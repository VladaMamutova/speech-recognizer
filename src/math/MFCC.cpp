...
double* MFCC::transform(const double* source, uint32_t start, uint32_t finish,
...
{
	complex<double>* fourierComplexRaw = new complex<double>[length];
	double* fourierRaw = new double[length];

	for (uint32_t k = 0; k < length; k++) {
		fourierComplexRaw[k] = complex<double>(0, 0);
		...
		for (uint32_t n = 0; n < length; n++) {
			...
			fourierComplexRaw[k] += f * w;
		}

		// As for magnitude, let's use Euclidean distance for its calculation
		fourierRaw[k] = sqrt(norm(fourierComplexRaw[k]));
	}

	delete[] fourierComplexRaw;
