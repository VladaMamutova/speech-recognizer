...
void SVD::calculate() {
	...
	for (int i = 0; i < m; ++i) {
		for (int j = 0; j < n; ++j) {
			...
			double tmp1;
			if ((size_t)j < singularValues.size()) {
...
Matrix SVD::lowRankApproximation(int rank) {
	int m = A.numRows();
	int n = A.numCols();
...
void SVD::printSVD() const {
	...
	Sigma.printMatrix();
	std::cout << std::endl;
	V.adjoint().printMatrix();
	std::cout << std::endl;
	std::cout << U * Sigma * V.adjoint() << std::endl;
	std::cout << std::endl;
}
